...
class RateLimiter:
    def __init__(self) -> None:
        self._budgets: Dict[str, OriginBudget] = {}
    ...
    def check_and_consume(self, origin_id: str, cost_override: float = 0.0) -> Tuple[bool, str, float]:
        """
        ...
        """
        self._purge_expired()
        if origin_id not in self._budgets:
            self._budgets[origin_id] = OriginBudget(origin_hash=origin_id)
        budget = self._budgets[origin_id]
...
class VERANav:
    ...
    def reveal(self, origin_ip: str, branch: str, user_agent: str = "", b2b_token: str = "") -> Dict[str, Any]:
        with self._lock:
            ...
            allowed, _, _ = self._limiter.check_and_consume(origin_id, cost_override=0.2)
            if not allowed:
                return {"status": "unavailable", "message": "Service temporairement indisponible."}
            if branch not in {"radio", "edge", "artist"}:
                return {"status": "invalid_branch", "message": "Branche non reconnue."}
